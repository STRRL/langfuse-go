...
func loadCl00k() (*tiktoken.Tiktoken, error) {
	...
	tempPath, err := os.MkdirTemp("langfuse-go", "cl100k_base.tiktoken")
	if err != nil {
		return nil, err
	}
	err = downloadOrCache("https://openaipublic.blob.core.windows.net/encodings/cl100k_base.tiktoken", tempPath)
	ranks, err := bpeLoader.LoadTiktokenBpe(tempPath)
	if err != nil {
		return nil, err
	}
